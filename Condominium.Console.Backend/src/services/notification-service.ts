...
export class NotificationService {
    ...
    constructor(datasource: DataSource) {
        this.notificationRepository = new BaseRepository.default(datasource, NotificationData);
    };

    async insertRecord(notification: Partial<NotificationData>) {
        try {
            const { identifiers } = await this.notificationRepository.insert(notification);
    ...
    async getRecords(email: string) {
        try {
            ...
            const data = await this.notificationRepository.findAll(where, { createdAt: "DESC" }, ["id","message","createdAt","viewed"])
