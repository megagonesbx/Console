...
export class IncidentService {
  ...
  constructor(datasource: DataSource) {
    this.incidentRepository = new BaseRepository.default(
      datasource,
      IncidentData
    );
  }
  ...
  async getRecord(
    ...
    try {
      const report: IncidentData | null = await this.incidentRepository.findOne(
        { id },
        false,
        select
      );
  ...
  async getRecords(page: number, size: number, user: string | undefined) {
    try {
